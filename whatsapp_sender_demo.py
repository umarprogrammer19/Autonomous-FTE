...
class WhatsAppSender:
    def __init__(self, use_test_token=False):
        # Load credentials from environment variables
        if use_test_token:
            self.api_token = "0IvyJHVeWVDSfrjUn4PYJb7xVPxlXxjW"  # Test token for demo
        else:
            self.api_token = os.getenv('WHAPI_TOKEN', 'your_actual_whapi_cloud_token_here')
...
def main(use_test_token=False):
    """Main function to run the WhatsApp sender"""
    print("WhatsApp Message Sender")
    print("=" * 30)

    try:
        sender = WhatsAppSender(use_test_token=use_test_token)
...
if __name__ == "__main__":
    # For demo purposes, you can use test token
    main(use_test_token=True)