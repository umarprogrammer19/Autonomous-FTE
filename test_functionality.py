...
def test_service_methods():
    ...
    try:
        ...
        assert hasattr(EmailService, 'send_email'), "EmailService.send_email method missing"
        print("[OK] EmailService.send_email exists")
        
        # Check WhatsAppService methods
        assert hasattr(WhatsAppService, 'send_message'), "WhatsAppService.send_message method missing"
        print("[OK] WhatsAppService.send_message exists")
        
        # Check AIPostService methods
        assert hasattr(AIPostService, 'generate_and_post_now'), "AIPostService.generate_and_post_now method missing"
        assert hasattr(AIPostService, 'get_latest_post'), "AIPostService.get_latest_post method missing"
        assert hasattr(AIPostService, 'run_scheduler_background'), "AIPostService.run_scheduler_background method missing"
...
def test_utility_functions():
    ...
    try:
        from utils import load_env, send_test_message, create_sample_config
...
def test_data_functions():
    ...
    try:
        ...
        from app import load_contacts, save_contacts
        
        # Test saving and loading contacts
        test_contacts = [
            {"name": "Test Contact", "phone": "+1234567890", "added_date": datetime.now().isoformat()}
        ]
...
def test_app_structure():
    ...
    try:
        ...
        required_functions = [
            'dashboard_page', 'email_sender_page', 'whatsapp_manager_page',
            'ai_post_generator_page', 'settings_page', 'sidebar_navigation'
        ]
