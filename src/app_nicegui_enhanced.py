...
def sidebar_navigation():
    ...
    with ui.column().classes("w-full p-4 bg-gray-50 min-h-screen"):
        ui.label("🤖 Social Media Dashboard").classes(
            "text-xl font-bold mb-6 text-gray-800"
        )
        ...
        for page_name, icon in zip(
            ...
            with ui.row().classes(
                "w-full items-center cursor-pointer p-3 rounded-lg hover:bg-gray-200 transition-all duration-200"
            ).on("click", lambda p=page_name: navigate_to_page(p)):
        ...
        with ui.card().classes(
            "w-full p-4 bg-gradient-to-r from-blue-50 to-indigo-50 shadow-sm"
        ):
            ui.label("System Status").classes("font-bold text-gray-700 mb-2")

            # Scheduler status
            status_color = (
                "text-green-600" if state.scheduler_running else "text-red-600"
            )
            ...
            # Last post time
            ui.label(f"Last Post: {state.last_post_time or 'Never'}").classes(
...
def dashboard_page():
    ...
    with ui.column().classes("w-full p-6"):
        ...
        with ui.row().classes("w-full justify-between items-center mb-8"):
            ui.label("📊 Social Media AI Employee `Dashboard").classes(
                "text-3xl font-bold text-gray-800"
            )
            ui.button("Refresh", on_click=lambda: ui.run_javascript("location.reload()")).classes(
        ...
        with ui.grid(columns=4).classes("w-full gap-6 mb-8"):
            # Emails Sent Card
            with ui.card().classes(
                "w-full p-6 bg-gradient-to-br from-blue-50 to-blue-100 shadow-md rounded-xl border-l-4 border-blue-500"
            ):
                with ui.row().classes("items-center"):
                    ui.icon("email").classes("text-3xl text-blue-500 mr-3")
                    ui.label("Emails Sent").classes(
                        "text-lg font-semibold text-gray-700"
                    )
                ui.label(str(analytics["emails_sent"])).classes(
                    "text-3xl font-bold text-blue-600 mt-2"
                )
                ui.label("Total emails sent successfully").classes(
                    "text-sm text-gray-600"
                )

            # WhatsApp Messages Card
            with ui.card().classes(
                "w-full p-6 bg-gradient-to-br from-green-50 to-green-100 shadow-md rounded-xl border-l-4 border-green-500"
            ):
                with ui.row().classes("items-center"):
                    ui.icon("chat").classes("text-3xl text-green-500 mr-3")
                    ui.label("WhatsApp Messages").classes(
                        "text-lg font-semibold text-gray-700"
                    )
                ui.label(str(analytics["whatsapp_messages"])).classes(
            ...
            with ui.card().classes(
                "w-full p-6 bg-gradient-to-br from-purple-50 to-purple-100 shadow-md rounded-xl border-l-4 border-purple-500"
            ):
                with ui.row().classes("items-center"):
                    ui.icon("auto_awesome").classes("text-3xl text-purple-500 mr-3")
                    ui.label("AI Posts Generated").classes(
                        "text-lg font-semibold text-gray-700"
                    )
                ui.label(str(analytics["ai_posts_generated"])).classes(
                    "text-3xl font-bold text-purple-600 mt-2"
                )
                ui.label("Posts created by AI").classes("text-sm text-gray-600")

            # Scheduler Status Card
            with ui.card().classes(
                "w-full p-6 bg-gradient-to-br from-indigo-50 to-indigo-100 shadow-md rounded-xl border-l-4 border-indigo-500"
            ):
                with ui.row().classes("items-center"):
                    ui.icon("schedule").classes("text-3xl text-indigo-500 mr-3")
                    ui.label("Scheduler Status").classes(
                        "text-lg font-semibold text-gray-700"
                    )
        ...
        with ui.card().classes(
            "w-full p-6 mb-8 bg-white shadow-md rounded-xl border border-gray-200"
        ):
            with ui.row().classes("w-full justify-between items-center"):
                ui.label("AI Scheduler Control").classes(
                ...
                ui.label(status_text).classes(
                    f"px-3 py-1 rounded-full text-sm font-medium {status_badge_color}"
                )

            with ui.row().classes("w-full gap-4 mt-4"):
                with ui.row().classes("gap-4"):
                    start_btn = ui.button(
                        "▶️ Start AI Scheduler", on_click=start_scheduler
                    ).classes(
                        "bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-200"
                    )
                    stop_btn = ui.button(
                        "⏹️ Stop AI Scheduler", on_click=stop_scheduler
                    ).classes(
                        "bg-gradient-to-r from-red-500 to-red-600 text-white px-6 py-3 rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-200"
                    )
        ...
        with ui.card().classes(
            "w-full p-6 bg-white shadow-md rounded-xl border border-gray-200"
        ):
            ...
            with ui.grid(columns=2).classes("w-full gap-4"):
                ...
                with ui.card().classes("p-4 bg-gray-50 rounded-lg"):
                    ui.label("Latest Generated Post").classes(
                    ...
                    if latest_post:
                        ui.label(
                            f"📝 {latest_post.get('content', 'No content available')[:100]}..."
                        ).classes("text-gray-600")
                        ui.label(
                            f"📅 {latest_post.get('timestamp', 'Unknown')}"
                        ).classes("text-sm text-gray-500")
                        ui.label(
                            f"📱 Platforms: {', '.join(latest_post.get('platforms', []))}"
                        ).classes("text-sm text-gray-500")
                    else:
                        ui.label("No posts generated yet.").classes(
                ...
                with ui.card().classes("p-4 bg-gray-50 rounded-lg"):
                    ...
                    ui.label(
                        f"🕒 Last Post: {state.last_post_time or 'Never'}"
                    ).classes("text-gray-600")
                    ui.label(
                        f"📈 Total Posts: {analytics['ai_posts_generated']}"
                    ).classes("text-gray-600")
                    ui.label(
                        f"🔔 Scheduler: {'Active' if state.scheduler_running else 'Inactive'}"
                    ).classes("text-gray-600")
...
def start_scheduler():
    ...
    if not state.scheduler_running:
        ...
        scheduler_thread = threading.Thread(
            target=AIPostService.run_scheduler_background
        )
...
def email_sender_page():
    ...
    with ui.column().classes("w-full p-6 max-w-4xl mx-auto"):
        ...
        # Description
        ui.label("Send professional emails directly from the dashboard").classes(
            "text-gray-600 mb-8 text-lg"
        )

        # Email form card
        with ui.card().classes(
            "w-full p-8 bg-white shadow-xl rounded-2xl border border-gray-200"
        ):
            with ui.column().classes("w-full"):
                recipient_email = ui.input(
                    "Recipient Email", placeholder="user@example.com"
                ).classes(
                    "w-full mb-6 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                )
                subject = ui.input("Subject", placeholder="Email subject").classes(
                    "w-full mb-6 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                )
                message = ui.textarea(
                    "Message", placeholder="Enter your message here..."
                ).classes(
                    "w-full mb-8 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                )
                message.props('rows="6"')

                async def send_email_handler():
                    if (
                        not recipient_email.value
                        or not subject.value
                        or not message.value
                    ):
                    ...
                    try:
                        ...
                        if result:
                            ...
                            ui.notify(
                                f"✅ Email sent successfully to {recipient_email.value}!",
                                type="positive",
                            )
        ...
        with ui.row().classes("w-full justify-center mt-6"):
            ui.button("📤 Send Email", on_click=send_email_handler).classes(
                "bg-gradient-to-r from-blue-500 to-blue-600 text-white px-8 py-3 rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-200"
            )
...
def whatsapp_manager_page():
    ...
    with ui.column().classes("w-full p-6 max-w-6xl mx-auto"):
        ...
        # Description
        ui.label("Manage contacts and send WhatsApp messages").classes(
            "text-gray-600 mb-8 text-lg"
        )

        # Create tabs for contact management and sending
        with ui.tabs().classes("w-full bg-gray-100 p-1 rounded-lg") as tabs:
            contact_tab = ui.tab("Contact Management", label="Contacts").classes(
                "p-4 rounded-lg"
            )
            send_tab = ui.tab("Send Message", label="Send Message").classes(
                "p-4 rounded-lg"
            )

        with ui.tab_panels(tabs, value=contact_tab).classes(
            "w-full mt-4 bg-white shadow-xl rounded-2xl p-6"
        ):
...
def contact_management_section():
    ...
    with ui.column().classes("w-full"):
        ...
        with ui.grid(columns=2).classes("w-full gap-6 mb-8"):
            name_input = ui.input("Name", placeholder="John Doe").classes(
                "w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
            )
            phone_input = ui.input("Phone Number", placeholder="+1234567890").classes(
                "w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
            )
        ...
        async def add_contact_handler():
            ...
            # Sanitize phone number: keep only digits and dashes
            clean_phone = "".join(
                c for c in str(phone_input.value).strip() if c.isdigit() or c == "-"
            )
            clean_name = str(name_input.value).strip()

            contacts = load_contacts()
            # Check if contact already exists
            contact_exists = any(c["phone"] == clean_phone for c in contacts)
            if contact_exists:
                ui.notify(
                    "Contact with this phone number already exists!", type="warning"
                )
            else:
                contacts.append(
                    {
                ...
                ui.notify(
                    f"✅ Contact {clean_name} added successfully!", type="positive"
                )
                name_input.value = ""
                phone_input.value = ""
        ...
        ui.button("Add Contact", on_click=add_contact_handler).classes(
            "bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-200 w-fit mb-8"
        )

        # Display all contacts
        contacts = load_contacts()
        if contacts:
            ui.label(f"All Contacts ({len(contacts)} total)").classes(
                "text-xl font-semibold text-gray-800 mt-6 mb-6"
            )

            with ui.column().classes("w-full"):
                for i, contact in enumerate(contacts):
                    with ui.card().classes(
                        "w-full p-4 bg-gray-50 rounded-lg border border-gray-200 mb-3"
                    ):
                        with ui.row().classes("w-full justify-between items-center"):
                            with ui.column():
                                ui.label(f"👤 {contact['name']}").classes(
                                    "font-semibold text-gray-700"
                                )
                                ui.label(f"📞 {contact['phone']}").classes(
                                    "text-gray-600 text-sm"
                                )
                                ui.label(
                                    f"📅 Added: {contact['added_date'][:10]}"
                                ).classes("text-gray-500 text-xs")

                            ui.button(
                                "Delete", on_click=lambda c=contact: delete_contact(c)
                            ).classes("bg-red-500 text-white px-3 py-1 rounded text-xs")
        else:
            ui.label("No contacts available. Please add contacts first.").classes(
...
def send_message_section():
    ...
    with ui.column().classes("w-full"):
        ui.label("Send WhatsApp Message").classes(
        ...
        if contacts:
            ...
            contact_select = ui.select(
                options=["Choose a contact..."] + contact_names, label="Select Contact"
            ).classes(
                "w-full mb-6 p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
            )
            ...
            async def send_whatsapp_handler():
                if (
                    contact_select.value == "Choose a contact..."
                    or not contact_select.value
                ):
                ...
                try:
                    ...
                    result = WhatsAppService.send_message(
                        selected_phone, message_area.value
                    )
                    if result:
                        increment_whatsapp_sent()  # Track WhatsApp message sent
                        ui.notify(
                            f"✅ Message sent to {contact_select.value}!",
                            type="positive",
                        )
                        message_area.value = ""  # Clear message after sending
                    else:
                        ui.notify(
                            "❌ Failed to send WhatsApp message. Please check the logs.",
                            type="negative",
                        )
                except Exception as e:
        ...
        if contacts:
            with ui.row().classes("w-full justify-center mt-6"):
                ui.button(
                    "Send WhatsApp Message", on_click=send_whatsapp_handler
                ).classes(
                    "bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-200"
                )
...
def ai_post_generator_page():
    ...
    with ui.column().classes("w-full p-6 max-w-4xl mx-auto"):
        # Page header
        ui.label("🤖 AI Post Generator").classes(
            "text-3xl font-bold text-gray-800 mb-8"
        )

        # Description
        ui.label("Generate and schedule social media posts with AI").classes(
            "text-gray-600 mb-8 text-lg"
        )

        # Auto Post Card
        with ui.card().classes(
            "w-full p-8 mb-8 bg-gradient-to-r from-purple-50 to-indigo-50 shadow-lg rounded-2xl border border-purple-200"
        ):
            ui.label("Automatic Posting").classes(
            ...
            with ui.row().classes("items-center gap-4"):
                ...
                ui.label("Enable automatic posting every 24 hours").classes(
                    "text-gray-700"
                )

            ui.label(
                "When enabled, AI will generate and post content automatically on a schedule"
        ...
        def toggle_auto_post(enabled):
            if enabled != state.scheduler_running:
                if enabled:
                    ...
                    scheduler_thread.start()
                    ui.notify(
                        "Automatic posting enabled! AI will post immediately, then every 24 hours.",
                        type="positive",
                    )
                else:
                    state.scheduler_running = False
        ...
        with ui.card().classes(
            ...
            if latest_post:
                ...
                ui.label(
                    f"{latest_post.get('content', 'No content available')}"
                ).classes("text-gray-600 mb-4 p-4 bg-gray-50 rounded-lg")

                ui.label(f"Timestamp:").classes("font-semibold text-gray-700")
                ui.label(f"{latest_post.get('timestamp', 'Unknown')}").classes(
                    "text-gray-600 mb-4"
                )
...
def settings_page():
    ...
    with ui.column().classes("w-full p-6 max-w-6xl mx-auto"):
        ...
        # Description
        ui.label("Configure application settings and view logs").classes(
            "text-gray-600 mb-8 text-lg"
        )

        # Application Settings Card
        with ui.card().classes(
            "w-full p-8 mb-8 bg-white shadow-xl rounded-2xl border border-gray-200"
        ):
            ...
            with ui.grid(columns=2).classes("w-full gap-6 mb-8"):
                with ui.card().classes("p-4 bg-gray-50 rounded-lg"):
                    ...
                    ui.label(f"Python Version: {sys.version.split()[0]}").classes(
                        "text-gray-600 text-sm"
                    )
                    import nicegui

                    ui.label(f"NiceGUI Version: {nicegui.__version__}").classes(
                        "text-gray-600 text-sm"
                    )
                    ui.label(f"Working Directory: {os.getcwd()}").classes(
                        "text-gray-600 text-sm"
                    )
                ...
                with ui.card().classes("p-4 bg-gray-50 rounded-lg"):
                    ...
                    analytics = get_analytics_data()
                    ui.label(f"Emails Sent: {analytics['emails_sent']}").classes(
                        "text-gray-600 text-sm"
                    )
                    ui.label(
                        f"WhatsApp Messages: {analytics['whatsapp_messages']}"
                    ).classes("text-gray-600 text-sm")
                    ui.label(
                        f"AI Posts Generated: {analytics['ai_posts_generated']}"
                    ).classes("text-gray-600 text-sm")

        # Log Viewer Card
        with ui.card().classes(
            "w-full p-8 bg-gray-900 text-gray-100 rounded-2xl shadow-xl"
        ):
            ui.label("Application Logs").classes(
            ...
            if os.path.exists("logs/app.log"):
                with open("logs/app.log", "r") as f:
                    logs = f.read()
                log_textarea = ui.textarea("Logs", value=logs).classes(
                    "w-full font-mono text-sm bg-gray-800 text-green-400 border border-gray-700"
                )
...
def main_page():
    with ui.grid(columns=12).classes("w-screen h-screen bg-gray-100"):
        ...
        with ui.column().classes(
            "col-span-2 bg-white shadow-lg min-h-screen p-4 border-r border-gray-200"
        ):
            sidebar_navigation()
