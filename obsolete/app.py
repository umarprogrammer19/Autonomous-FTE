...
def sidebar_navigation():
    ...
    with ui.column().classes('w-full p-4'):
        ...
        for page in pages:
            # Create buttons with proper click handlers
            btn = ui.button(page, on_click=lambda p=page: navigate_to_page(p)).classes('w-full mb-2')
...
# Global reference to the content holder so we can update it from anywhere
content_holder_ref = None

# The navigate_to_page function is now defined later in the file with the refreshable approach
    ...
    with ui.column().classes('w-full p-4'):
        ...
        with ui.column().classes('mb-6'):
            ...
            with ui.row().classes('gap-4'):
                ...
                # Bind button visibility based on scheduler status
                start_btn.bind_visibility_from(state, 'scheduler_running', backward=lambda x: not x)
                stop_btn.bind_visibility_from(state, 'scheduler_running', backward=lambda x: x)
...
def start_scheduler():
    ...
    if not state.scheduler_running:
        ...
        # Update the UI by refreshing the content area
    ...
    with ui.column().classes('w-full'):
        ...
        async def add_contact_handler():
            ...
            if contact_exists:
                ...
                name_input.value = ""
                phone_input.value = ""
                # Refresh the content area to update the UI
                content_area.refresh()
    ...
    with ui.column().classes('w-full p-4 max-w-4xl mx-auto'):
        ...
        if os.path.exists('logs/app.log'):
            ...
            with open('logs/app.log', 'r') as f:
                logs = f.read()
            ui.textarea("Logs", value=logs).classes('w-full font-mono text-sm').props('rows="15"')
        else:
            ui.label("No log file found.").classes('text-gray-500')

@ui.refreshable
def content_area():
    """Refreshable content area that updates based on current page"""
...
def navigate_to_page(page_name):
    ...
    content_area.refresh()  # Refresh the content area
...
def main_page():
    with ui.grid(columns=12).classes('w-screen h-screen'):
        ...
        # Main content area that updates based on current page
        content_area()  # Initial call to the refreshable content area
